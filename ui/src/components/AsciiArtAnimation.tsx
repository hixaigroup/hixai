...
type PaperclipSprite = (typeof HIXAI_SPRITES)[number];
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = HIXAI_SPRITES[(baseCol + baseRow) % HIXAI_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = HIXAI_SPRITES[Math.floor(Math.random() * HIXAI_SPRITES.length)]!;
