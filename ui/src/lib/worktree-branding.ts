...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("hixai-worktree-enabled") !== "true") return null;

  const name = readMetaContent("hixai-worktree-name");
  const color = normalizeHexColor(readMetaContent("hixai-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("hixai-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
