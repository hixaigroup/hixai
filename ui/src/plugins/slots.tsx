 ...
 *
 * Plugin bundles are built with `external: ["@hixai/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__hixaiPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__hixaiPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__hixaiPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__hixaiPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@hixai/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@hixai/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@hixai/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@hixai/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@hixai/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__hixaiPluginBridge__) {
 ...
 * self-register via `window.hixaiPlugins.registerReactComponent()`.
 ...
 * of `@hixai/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
