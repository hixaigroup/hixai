...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@hixai/db";
import type { DeploymentMode } from "@hixai/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  hixaiUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).hixaiUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.hixaiUpgradeContext = context;
