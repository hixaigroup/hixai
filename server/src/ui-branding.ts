const FAVICON_BLOCK_START = "<!-- HIXAI_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- HIXAI_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- HIXAI_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- HIXAI_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.HIXAI_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.HIXAI_WORKTREE_NAME) ?? nonEmpty(env.HIXAI_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.HIXAI_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="hixai-worktree-enabled" content="true" />',
    `<meta name="hixai-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="hixai-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="hixai-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
