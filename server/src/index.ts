...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.HIXAI_SECRETS_PROVIDER === undefined) {
    process.env.HIXAI_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.HIXAI_SECRETS_STRICT_MODE === undefined) {
    process.env.HIXAI_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.HIXAI_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.HIXAI_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.HIXAI_MIGRATION_PROMPT === "never") return false;
    if (process.env.HIXAI_MIGRATION_AUTO_APPLY === "true") return true;
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@hixai.local";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.HIXAI_EMBEDDED_POSTGRES_VERBOSE === "true";
    ...
    const embeddedAdminConnectionString = `postgres://hixai:hixai@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "hixai");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: hixai");
    }
  
    const embeddedConnectionString = `postgres://hixai:hixai@127.0.0.1:${port}/hixai`;
  ...
  if (config.deploymentMode === "authenticated") {
    ...
    const betterAuthSecret =
      process.env.BETTER_AUTH_SECRET?.trim() ?? process.env.HIXAI_AGENT_JWT_SECRET?.trim();
    if (!betterAuthSecret) {
      throw new Error(
        "authenticated mode requires BETTER_AUTH_SECRET (or HIXAI_AGENT_JWT_SECRET) to be set",
      );
    }
  ...
  const runtimeApiHost =
    runtimeListenHost === "0.0.0.0" || runtimeListenHost === "::"
      ? "localhost"
      : runtimeListenHost;
  process.env.HIXAI_LISTEN_HOST = runtimeListenHost;
  process.env.HIXAI_LISTEN_PORT = String(listenPort);
  process.env.HIXAI_API_URL = `http://${runtimeApiHost}:${listenPort}`;
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.HIXAI_OPEN_ON_LISTEN === "true") {
  ...
  return {
    ...
    host: config.host,
    listenPort,
    apiUrl: process.env.HIXAI_API_URL ?? `http://${runtimeApiHost}:${listenPort}`,
