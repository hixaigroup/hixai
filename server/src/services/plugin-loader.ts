 ...
 *    (`~/.hixai/plugins/`) and `node_modules` for packages matching
 *    the `hixai-plugin-*` naming convention. Aggregates results with
 ...
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "hixai-plugin-";
...
export type PluginSource =
  | "local-filesystem"  // ~/.hixai/plugins/ local directory
  | "npm"               // npm packages matching hixai-plugin-* convention
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the hixai-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "hixai-plugin-linear" or "@acme/plugin-linear").
...
export function isPluginPackageName(name: string): boolean {
  ...
  // Also accept scoped packages like @acme/plugin-linear or @hixai/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "hixaiPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const hixaiPlugin = pkgJson["hixaiPlugin"];
  if (
    hixaiPlugin !== null &&
    typeof hixaiPlugin === "object" &&
    !Array.isArray(hixaiPlugin)
  ) {
    const manifestRelPath = (hixaiPlugin as Record<string, unknown>)[
      "manifest"
    ];
 ...
 *   packageName: "hixai-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @hixai/shared exports). Run those workers through
