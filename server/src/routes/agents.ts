...
import { agents as agentsTable, companies, heartbeatRuns } from "@hixai/db";
...
import { runClaudeLogin } from "@hixai/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@hixai/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@hixai/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@hixai/adapter-gemini-local";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@hixai/adapter-opencode-local/server";
...
export function agentRoutes(db: Db) {
  ...
  const strictSecretsMode = process.env.HIXAI_SECRETS_STRICT_MODE === "true";
