...
import type { DeploymentExposure, DeploymentMode } from "@hixai/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    hixaiApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.hixaiApiUrl)
      : null,
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm hixai allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/hixai";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/hixai-claimed-api-key.json and load HIXAI_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: hixaiApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/hixai/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          hixaiApiUrl: "http://host.docker.internal:3100",
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "hixaiApiUrl": "https://hixai-hostname-your-agent-can-reach:3100",
    ...
    - ~/.openclaw/workspace/hixai-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/hixai-claimed-api-key.json

    And set the HIXAI_API_KEY and HIXAI_API_URL in your environment variables as specified here:
    ...
    Then set HIXAI_API_KEY and HIXAI_API_URL from the saved token field for every heartbeat run.
    ...
    Be sure to prepend your HIXAI_API_URL to the top of your skill and note the path to your HIXAI_API_URL
  ...
  if (connectionCandidates.length > 0) {
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.hixaiApiUrl when submitting your join request
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@hixai.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "hixai", path: "/api/skills/hixai" },
        {
        ...
        },
        {
          ...
          path: "/api/skills/hixai-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              hixaiApiUrl: req.body.hixaiApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.hixaiApiUrl &&
          !persistedDefaults.hixaiApiUrl
        ) {
          missingPersistedFields.push("hixaiApiUrl");
        }
