...
function jwtConfig() {
  const secret = process.env.HIXAI_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.HIXAI_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.HIXAI_AGENT_JWT_ISSUER ?? "hixai",
    audience: process.env.HIXAI_AGENT_JWT_AUDIENCE ?? "hixai-api",
  };
}
