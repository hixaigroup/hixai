...
import { isCursorUnknownSessionError, parseCursorJsonl } from "@hixai/adapter-cursor-local/server";
import { parseCursorStdoutLine } from "@hixai/adapter-cursor-local/ui";
import { printCursorStreamEvent } from "@hixai/adapter-cursor-local/cli";
...
describe("cursor ui stdout parser", () => {
  ...
  it("compacts shellToolCall and shell tool result for run log", () => {
    const ts = "2026-03-05T00:00:00.000Z";
    const longCommand = "curl -s -X POST \"$HIXAI_API_URL/api/issues/abc/checkout\" -H \"Authorization: Bearer $HIXAI_API_KEY\"";
