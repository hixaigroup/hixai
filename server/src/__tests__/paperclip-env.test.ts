...
const ORIGINAL_HIXAI_LISTEN_PORT = process.env.HIXAI_LISTEN_PORT;
const ORIGINAL_HOST = process.env.HOST;
const ORIGINAL_PORT = process.env.PORT;

afterEach(() => {
  if (ORIGINAL_HIXAI_API_URL === undefined) delete process.env.HIXAI_API_URL;
  else process.env.HIXAI_API_URL = ORIGINAL_HIXAI_API_URL;

  if (ORIGINAL_HIXAI_LISTEN_HOST === undefined) delete process.env.HIXAI_LISTEN_HOST;
  else process.env.HIXAI_LISTEN_HOST = ORIGINAL_HIXAI_LISTEN_HOST;

  if (ORIGINAL_HIXAI_LISTEN_PORT === undefined) delete process.env.HIXAI_LISTEN_PORT;
  else process.env.HIXAI_LISTEN_PORT = ORIGINAL_HIXAI_LISTEN_PORT;
...
describe("buildHIxAIEnv", () => {
  ...
  it("uses runtime listen host/port when explicit URL is not set", () => {
    delete process.env.HIXAI_API_URL;
    process.env.HIXAI_LISTEN_HOST = "0.0.0.0";
    process.env.HIXAI_LISTEN_PORT = "3101";
