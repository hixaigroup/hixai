...
describe("agent local JWT", () => {
  const secretEnv = "HIXAI_AGENT_JWT_SECRET";
  const ttlEnv = "HIXAI_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "HIXAI_AGENT_JWT_ISSUER";
  const audienceEnv = "HIXAI_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "hixai";
    process.env[audienceEnv] = "hixai-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
