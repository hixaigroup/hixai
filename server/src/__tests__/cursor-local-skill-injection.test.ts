...
import { ensureCursorSkillsInjected } from "@hixai/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing HIxAI skills into Cursor skills home", async () => {
    ...
    await createSkillDir(skillsDir, "hixai");
    await createSkillDir(skillsDir, "hixai-create-agent");
    ...
    const injectedA = path.join(skillsHome, "hixai");
    const injectedB = path.join(skillsHome, "hixai-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "hixai")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "hixai-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "hixai"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "hixai-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("hixai-cursor-preserve-src-");
    const skillsHome = await makeTempDir("hixai-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "hixai");
    ...
    expect((await fs.lstat(path.join(skillsHome, "hixai-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("hixai-cursor-fail-src-");
    const skillsHome = await makeTempDir("hixai-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
