...
<head>
    <!-- HIXAI_RUNTIME_BRANDING_START -->
    <!-- HIXAI_RUNTIME_BRANDING_END -->
    <!-- HIXAI_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from HIXAI_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ HIXAI_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ HIXAI_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ HIXAI_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      HIXAI_IN_WORKTREE: "true",
      HIXAI_WORKTREE_NAME: "hixai-pr-432",
      HIXAI_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("hixai-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="hixai-worktree-name"');
    expect(meta).toContain('content="hixai-pr-432"');
    expect(meta).toContain('name="hixai-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="hixai-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="hixai-worktree-name"');
  });
});
