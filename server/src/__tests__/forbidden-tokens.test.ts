...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "hixai", LOGNAME: "hixai", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "hixai" }),
      },
    );

    expect(tokens).toEqual(["hixai", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "hixai" }, {
        userInfo: () => ({ username: "hixai" }),
      });

      expect(tokens).toEqual(["hixai", "custom-token"]);
    } finally {
