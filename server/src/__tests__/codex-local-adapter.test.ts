...
import { isCodexUnknownSessionError, parseCodexJsonl } from "@hixai/adapter-codex-local/server";
import { parseCodexStdoutLine } from "@hixai/adapter-codex-local/ui";
import { printCodexStreamEvent } from "@hixai/adapter-codex-local/cli";
  ...
  it("parses turn and reasoning lifecycle events", () => {
    ...
    expect(
      parseCodexStdoutLine(
        JSON.stringify({
          type: "item.completed",
          item: { id: "item_1", type: "reasoning", text: "**Preparing to use hixai skill**" },
        }),
        ts,
      ),
    ).toEqual([
      { kind: "thinking", ts, text: "**Preparing to use hixai skill**" },
    ]);
  });
      ...
      parseCodexStdoutLine(
        JSON.stringify({
          ...
          item: {
            id: "item_52",
            type: "file_change",
            changes: [{ path: "/Users/hixaiuser/project/ui/src/pages/AgentDetail.tsx", kind: "update" }],
            status: "completed",
          },
        }),
        ts,
