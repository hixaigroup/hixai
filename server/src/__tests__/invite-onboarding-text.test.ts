...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("HIxAI OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested HIxAI base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("hixaiApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.hixaiApiUrl");
    expect(text).toContain("~/.openclaw/workspace/hixai-claimed-api-key.json");
    expect(text).toContain("HIXAI_API_KEY");
