...
import type { AdapterModel } from "@hixai/adapter-utils";
import { asString, runChildProcess } from "@hixai/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.HIXAI_PI_COMMAND === "string" &&
    process.env.HIXAI_PI_COMMAND.trim().length > 0
      ? process.env.HIXAI_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["HIXAI_", "npm_", "NPM_"] as const;
