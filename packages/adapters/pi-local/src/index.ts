...
Use when:
- You want HIxAI to run Pi (the AI coding agent) locally as the agent runtime
...
- HIxAI requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/hixais/ and resumed with --session.
