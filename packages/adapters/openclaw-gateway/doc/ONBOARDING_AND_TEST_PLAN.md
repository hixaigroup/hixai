...
## Scope
This plan is now **gateway-only**. HIxAI supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses HIxAI skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a HIxAI task.
...
```bash
pnpm --filter @hixai/server test:run -- openclaw-gateway
pnpm --filter @hixai/server typecheck
pnpm --filter @hixai/ui typecheck
pnpm --filter hixai typecheck
```
