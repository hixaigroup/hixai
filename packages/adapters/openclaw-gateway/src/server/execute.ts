...
const DEFAULT_CLIENT_VERSION = "hixai";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "hixai";
  if (input.strategy === "run") return `hixai:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `hixai:issue:${input.issueId}`;
  return fallback;
}
...
function buildHIxAIEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  ...
  if (hixaiApiUrlOverride) {
    hixaiEnv.HIXAI_API_URL = hixaiApiUrlOverride;
  }
  if (wakePayload.taskId) hixaiEnv.HIXAI_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) hixaiEnv.HIXAI_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) hixaiEnv.HIXAI_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) hixaiEnv.HIXAI_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) hixaiEnv.HIXAI_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    hixaiEnv.HIXAI_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return hixaiEnv;
}

function buildWakeText(payload: WakePayload, hixaiEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/hixai-claimed-api-key.json";
  const orderedKeys = [
    "HIXAI_RUN_ID",
    "HIXAI_AGENT_ID",
    "HIXAI_COMPANY_ID",
    ...
    "HIXAI_TASK_ID",
    "HIXAI_WAKE_REASON",
    "HIXAI_WAKE_COMMENT_ID",
    "HIXAI_APPROVAL_ID",
    "HIXAI_APPROVAL_STATUS",
    "HIXAI_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = hixaiEnv[key];
    if (!value) continue;
  ...
  const lines = [
    ...
    ...envLines,
    `HIXAI_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load HIXAI_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: HIXAI_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$HIXAI_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$HIXAI_COMPANY_ID/issues?assigneeAgentId=$HIXAI_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardHIxAIPayload(
  ...
  const runtimeServiceIntents = Array.isArray(ctx.context.hixaiRuntimeServiceIntents)
    ? ctx.context.hixaiRuntimeServiceIntents.filter(
  ...
  const standardHIxAI: Record<string, unknown> = {
    ...
    apiUrl: hixaiEnv.HIXAI_API_URL ?? null,
  };
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "hixai-complete");
    this.ws = null;
  }
