...
const HIXAI_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface HixAISkillEntry {
  name: string;
  source: string;
}
...
export function buildHixAIEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const runtimeHost = resolveHostForUrl(
    process.env.HIXAI_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.HIXAI_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.HIXAI_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.HIXAI_API_URL = apiUrl;
  return vars;
}
...
export async function resolveHixAISkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...HIXAI_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listHixAISkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<HixAISkillEntry[]> {
  const root = await resolveHixAISkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
...
export async function readHixAISkillMarkdown(
  moduleDir: string,
  skillName: string,
): Promise<string | null> {
...
export async function ensureHixAISkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the HixAI server itself is started from
    // within a Claude Code session (e.g. `npx hixai run` in a terminal
