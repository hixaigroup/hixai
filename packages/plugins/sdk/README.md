...
- **Testing:** `@hixai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@hixai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@hixai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@hixai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@hixai/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@hixai/plugin-sdk/ui/hooks` | Hooks only |
| `@hixai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@hixai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@hixai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@hixai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@hixai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@hixai/plugin-sdk/types` | Worker context and API types (advanced) |
...
```ts
import { definePlugin, runWorker } from "@hixai/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@hixai/plugin-sdk`.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@hixai/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@hixai/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@hixai/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@hixai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@hixai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@hixai/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@hixai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@hixai/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@hixai/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@hixai/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@hixai/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@hixai/plugin-sdk/bundlers";
...
```bash
hixai-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@hixai/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__hixai__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__hixai__/events` streams `reload` SSE events on UI build changes
