...
```bash
npx @hixai/create-hixai-plugin @acme/my-plugin \
  --template connector \
  --category connector \
...
- example UI widget using the supported `@hixai/plugin-sdk/ui` hooks
- test file using `@hixai/plugin-sdk/testing`
- `esbuild` and `rollup` config files using SDK bundler presets
- dev server script for hot-reload (`hixai-plugin-dev-server`)
...
node packages/plugins/create-hixai-plugin/dist/index.js @acme/my-plugin \
  ...
  --sdk-path /absolute/path/to/hixai/packages/plugins/sdk
```
