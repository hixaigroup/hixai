...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".hixai-sdk");
...
/**
 * Generate a complete HIxAI plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A HIxAI plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "hixai-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["hixai", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@hixai/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@hixai/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@hixai/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@hixai/plugin-sdk\` and \`@hixai/shared\` from a local HIxAI checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.hixai-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-hixai-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <hixai-sdk-path>]");
    process.exit(1);
  }
