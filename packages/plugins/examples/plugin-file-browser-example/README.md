...
Example HixAI plugin that demonstrates:
...
```bash
pnpm --filter @hixai/plugin-file-browser-example build
pnpm hixai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm hixai plugin uninstall hixai-file-browser-example --force
```
...
- Optional: use `hixai-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
