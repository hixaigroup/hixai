# HIxAI Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for HIxAI's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main HIxAI app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary HIxAI HTTP APIs directly.
...
HIxAI plugin design is based on the following assumptions:

1. HIxAI is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core HIxAI business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by HIxAI core.
...
2. Let plugins add major capabilities without editing HIxAI core.
...
4. Depend on project-local plugin folders such as `.hixai/plugins`.
...
The single HIxAI deployment an operator installs and controls.

### 5.2 Company

A first-class HIxAI business object inside the instance.
...
A trusted in-process extension loaded directly by HIxAI core.
...
An installable instance-wide extension package loaded through the HIxAI plugin runtime.
...
HIxAI has two extension classes.
...
HIxAI already has a concrete workspace model:
...
Plugins live under the HIxAI instance directory.

Suggested layout:

- `~/.hixai/instances/default/plugins/package.json`
- `~/.hixai/instances/default/plugins/node_modules/`
- `~/.hixai/instances/default/plugins/.cache/`
- `~/.hixai/instances/default/data/plugins/<plugin-id>/`
...
HIxAI should add CLI commands:

- `pnpm hixai plugin list`
- `pnpm hixai plugin install <package[@version]>`
- `pnpm hixai plugin uninstall <plugin-id>`
- `pnpm hixai plugin upgrade <plugin-id> [version]`
- `pnpm hixai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@hixai/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
Plugins may contribute tools that HIxAI agents can use during runs.
...
- HIxAI server starts one worker process per installed plugin
...
Receives one typed HIxAI domain event.
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@hixai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@hixai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@hixai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@hixai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@hixai/plugin-sdk/ui`
...
The `@hixai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the HIxAI secret provider system rather than a plain text input
...
1. Core HIxAI data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing HIxAI objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual HIxAI product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if HIxAI later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing HIxAI secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm hixai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the HIxAI server. This is a normative requirement, not optional.
...
### 27.1 `@hixai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@hixai/plugin-test-harness";
...
For developing a plugin against a running HIxAI instance:

- The operator installs the plugin from a local path: `pnpm hixai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-hixai-plugin`) that scaffolds:

- `package.json` with correct `hixaiPlugin` keys
...
- `@hixai/plugin-workspace-files`
- `@hixai/plugin-terminal`
- `@hixai/plugin-git`
- `@hixai/plugin-linear`
- `@hixai/plugin-github-issues`
- `@hixai/plugin-grafana`
- `@hixai/plugin-runtime-processes`
- `@hixai/plugin-stripe`
...
- `@hixai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@hixai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@hixai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@hixai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@hixai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@hixai/plugin-sdk/ui`
...
- `@hixai/plugin-test-harness`
- `create-hixai-plugin` starter template
...
HIxAI should not implement a generic in-process hook bag modeled directly after local coding tools.

HIxAI should implement:
...
That is the complete target design for the HIxAI plugin system.
