...
Agents need API keys to authenticate with HIxAI. The current approach
...
**Trust model:** The adapter process runs on the same machine as the HIxAI
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** HIxAI generates a token and passes it directly to the agent
...
`HIXAI_API_URL`, `HIXAI_AGENT_ID`, `HIXAI_COMPANY_ID`. We need to
add a `HIXAI_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a HIxAI CLI
...
```
hixai auth login
# Opens browser -> user confirms -> token stored at ~/.hixai/credentials
```
...
managed by the HIxAI server itself.
...
1. A company admin (user or agent) generates an **invite URL** from HIxAI.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The HIxAI SKILL.md (or a link to it)
   - What information HIxAI needs from the agent (e.g. webhook URL, adapter
...
5. HIxAI stores the pending registration.
...
7. On approval, HIxAI provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** HIxAI issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.hixai.com/skills/hixai/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How HIxAI should send you heartbeats",
...
1. Generate a HIxAI invite link for the company.
...
4. A HIxAI company member approves the new agent.
5. HIxAI begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `HIXAI_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `hixai auth login` for developer-managed remote agents.                                      |
