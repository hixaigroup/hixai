...
`PAP-447` asks how HixAI should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same HixAI instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into HixAI:

- some operators use HixAI to manage HixAI and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as HixAI
- Claude and Codex expose different built-in affordances, so HixAI should not overfit to one tool

## Core Product Decision

HixAI should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by HixAI
...
- HixAI should not assume that every adapter can see or use a host filesystem path directly
...
If HixAI makes worktrees agent-first, it will blur:
...
- HixAI core resolves and records execution workspace state
...
That gives HixAI room to support:
...
- HixAI handles workspace runtime services under the hood using project/workspace policy
...
Once HixAI is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that HixAI cannot infer
...
- when issue moves to done, should HixAI auto-commit?
- should it auto-push?
...
HixAI already has the right foundation for a project-first model.
...
For server-spawned local adapters, HixAI already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that HixAI should mirror directly

Implication:

- for `codex_local`, HixAI should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but HixAI should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, HixAI should separate:
...
HixAI should be able to express intentions such as:
...
These are not specific to HixAI. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".hixai/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a HixAI-specific `devServer` flag.
...
HixAI should distinguish between:
...
HixAI should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, HixAI should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to HixAI
...
For runtime services specifically, HixAI should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as HixAI-only dev-server flags
...
3. Inject resolved execution workspace details into `context.hixaiWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / HixAI instance can be reused through env/config injection when appropriate
...
- HixAI can decide whether to reuse or start a fresh service deterministically
...
3. Test against HixAI-on-HixAI workflows first.
...
7. Workspace runtime services are modeled generically, not as HixAI-specific dev-server toggles.
