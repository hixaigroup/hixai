...
This document defines the V1 design for a HixAI adapter that integrates with
...
4. synthesized stdout events for HixAI UI/CLI
...
3. **Agent callback auth to HixAI** uses a bootstrap exchange flow (no long-lived HixAI key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from HixAI endpoints, not full SKILL.md prompt injection.
...
- `hixaiPublicUrl` (optional override; else `HIXAI_PUBLIC_URL` env)
...
## HixAI Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `HIXAI_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.hixaiPublicUrl`
2. `process.env.HIXAI_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived HixAI credentials in prompt text.

Flow:

1. Before launch/follow-up, HixAI mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. HixAI validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all HixAI API calls.
...
1. Prompt includes a compact instruction to fetch skills from HixAI.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/hixai`
   - `GET /api/skills/hixai-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <hixaiPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active HixAI run context.
...
5. `hixaiPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from HixAI skill endpoints |
| HixAI auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from HixAI context.
3. Add richer PR metadata surfacing in HixAI UI.
