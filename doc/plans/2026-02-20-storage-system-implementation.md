...
Add a single storage subsystem for HixAI that supports:
...
- API serves attachment bytes through authenticated HixAI endpoints.
...
- Default local path is under instance root: `~/.hixai/instances/<instanceId>/data/storage`.
...
- `hixai onboard` writes a valid `storage` config block by default.
- `hixai configure --section storage` can switch between local and s3 modes.
...
- `hixai doctor` reports actionable storage status.
