...
HIxAI's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends HIxAI's API, UI, and data model | Yes |
...
modules/
  observability/
    hixai.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@hixai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@hixai/core";
...
```
1. Scan modules/ directory for hixai.module.json manifests
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@hixai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@hixai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:hixai/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.hixai.com/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm hixai store list                    # browse available modules and templates
pnpm hixai store install <module-id>     # install a module
pnpm hixai store import <template-id>    # import a company template
pnpm hixai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one HIxAI instance | `server:started` |
...
4. **Module config** — `hixai.config.json` with per-module config, env var interpolation
...
10. **`@hixai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
