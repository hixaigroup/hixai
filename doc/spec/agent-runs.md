...
This spec defines how HixAI actually runs agents while staying runtime-agnostic.
...
1. HixAI is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, HixAI manages lifecycle, and on exit HixAI parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `hixai.skill` (shared HixAI skill text block)
...
2. Preferred transport is env vars (`HIXAI_*`) injected at runtime.
