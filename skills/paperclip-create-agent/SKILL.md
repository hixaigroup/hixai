...
```sh
curl -sS "$HIXAI_API_URL/api/agents/me" \
...
```sh
curl -sS "$HIXAI_API_URL/llms/agent-configuration/claude_local.txt" \
  -H "Authorization: Bearer $HIXAI_API_KEY"
```
...
```sh
curl -sS "$HIXAI_API_URL/api/companies/$HIXAI_COMPANY_ID/agent-configurations" \
  -H "Authorization: Bearer $HIXAI_API_KEY"
```
...
```sh
curl -sS "$HIXAI_API_URL/llms/agent-icons.txt" \
  -H "Authorization: Bearer $HIXAI_API_KEY"
```
...
```sh
curl -sS -X POST "$HIXAI_API_URL/api/companies/$HIXAI_COMPANY_ID/agent-hires" \
  -H "Authorization: Bearer $HIXAI_API_KEY" \
...
- when the board approves, you will be woken with `HIXAI_APPROVAL_ID`; read linked issues and close/comment follow-up

```sh
curl -sS "$HIXAI_API_URL/api/approvals/<approval-id>" \
  -H "Authorization: Bearer $HIXAI_API_KEY"

curl -sS -X POST "$HIXAI_API_URL/api/approvals/<approval-id>/comments" \
  -H "Authorization: Bearer $HIXAI_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS -X POST "$HIXAI_API_URL/api/issues/<issue-id>/approvals" \
  -H "Authorization: Bearer $HIXAI_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS "$HIXAI_API_URL/api/approvals/$HIXAI_APPROVAL_ID" \
  -H "Authorization: Bearer $HIXAI_API_KEY"

curl -sS "$HIXAI_API_URL/api/approvals/$HIXAI_APPROVAL_ID/issues" \
  -H "Authorization: Bearer $HIXAI_API_KEY"
```
...
`skills/hixai-create-agent/references/api-reference.md`
