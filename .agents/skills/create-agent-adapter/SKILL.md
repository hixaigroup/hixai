...
description: >
  Technical guide for creating a new HixAI agent adapter. Use when building
...
# Creating a HixAI Agent Adapter

An adapter bridges HixAI's orchestration layer to a specific AI agent runtime (Claude Code, Codex CLI, a custom process, an HTTP endpoint, etc.). Each adapter is a self-contained package that provides implementations for **three consumers**: the server, the UI, and the CLI.
...
packages/adapters/<name>/
  src/
    ...
    cli/
      ...
      format-event.ts   # Colored terminal output for `hixai run --watch`
  package.json
  tsconfig.json
```
...
## 2. Shared Types (`@hixai/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@hixai/adapter-utils` (types) or `@hixai/adapter-utils/server-utils` (runtime helpers).
...
interface AdapterExecutionResult {
  ...
  clearSession?: boolean;         // true = tell HixAI to forget the stored session
}
...
The `agentConfigurationDoc` is read by LLM agents (including HixAI agents that create other agents). Write it as **routing logic**, not marketing copy. Include concrete "use when" and "don't use when" guidance so an LLM can decide whether this adapter is appropriate for a given task.
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@hixai/adapter-utils/server-utils`)
2. **Build environment** — call `buildHixAIEnv(agent)` then layer in `HIXAI_RUN_ID`, context vars (`HIXAI_TASK_ID`, `HIXAI_WAKE_REASON`, `HIXAI_WAKE_COMMENT_ID`, `HIXAI_APPROVAL_ID`, `HIXAI_APPROVAL_STATUS`, `HIXAI_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| Variable | Source |
|----------|--------|
| `HIXAI_AGENT_ID` | `agent.id` |
| `HIXAI_COMPANY_ID` | `agent.companyId` |
| `HIXAI_API_URL` | Server's own URL |
| `HIXAI_RUN_ID` | Current run id |
| `HIXAI_TASK_ID` | `context.taskId` or `context.issueId` |
| `HIXAI_WAKE_REASON` | `context.wakeReason` |
| `HIXAI_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `HIXAI_APPROVAL_ID` | `context.approvalId` |
| `HIXAI_APPROVAL_STATUS` | `context.approvalStatus` |
| `HIXAI_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `HIXAI_API_KEY` | `authToken` (if no explicit key in config) |
...
Pretty-prints stdout lines for `hixai run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@hixai/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@hixai/adapter-my-agent";
...
const myAgentAdapter: ServerAdapterModule = {
  ...
  models: myModels,
  supportsLocalAgentJwt: true,  // true if agent can use HixAI API
...
import { parseMyAgentStdoutLine } from "@hixai/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@hixai/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@hixai/adapter-my-agent/cli";
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so HixAI wipes the stale session
...
Import from `@hixai/adapter-utils/server-utils`:
...
| `buildHixAIEnv(agent)` | Standard `HIXAI_*` env vars |
...
- Package name: `@hixai/adapter-<kebab-name>`
...
- Default prompt: `"You are agent {{agent.id}} ({{agent.name}}). Continue your HixAI work."`
...
### HixAI Skills Injection

HixAI ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `hixai` API skill and the `hixai-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.

**The constraint:** never copy or symlink skills into the agent's `cwd`. The cwd is the user's project checkout — writing `.claude/skills/` or any other files into it would contaminate the repo with HixAI internals, break git status, and potentially leak into commits.

**The pattern:** create a clean, isolated location for skills and tell the agent runtime to look there.

**How claude-local does it:**

1. At execution time, create a fresh tmpdir: `mkdtemp("hixai-skills-")`
...
async function buildSkillsDir(): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "hixai-skills-"));
  const target = path.join(tmp, ".claude", "skills");
  await fs.mkdir(target, { recursive: true });
  const entries = await fs.readdir(HIXAI_SKILLS_DIR, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(HIXAI_SKILLS_DIR, entry.name),
...
Codex has a global personal skills directory (`$CODEX_HOME/skills` or `~/.codex/skills`). The adapter symlinks HixAI skills there if they don't already exist. This is acceptable because it's the agent tool's own config directory, not the user's project.
...
**Skills as loaded procedures, not prompt bloat.** The HixAI skills (like `hixai` and `hixai-create-agent`) are designed as on-demand procedures: the agent sees skill metadata (name + description) in its context, but only loads the full SKILL.md content when it decides to invoke a skill. This keeps the base prompt small. When writing `agentConfigurationDoc` or prompt templates for your adapter, do not inline skill content — let the agent runtime's skill discovery do the work. The descriptions in each SKILL.md frontmatter act as routing logic: they tell the agent when to load the full skill, not what the skill contains.

**Explicit vs. fuzzy skill invocation.** For production workflows where reliability matters (e.g. an agent that must always call the HixAI API to report status), use explicit instructions in the prompt template: "Use the hixai skill to report your progress." Fuzzy routing (letting the model decide based on description matching) is fine for exploratory tasks but unreliable for mandatory procedures.

---

## 8. Security Considerations

Adapters sit at the boundary between HixAI's orchestration layer and arbitrary agent execution. This is a high-risk surface.
...
- `HIXAI_API_KEY` is injected by the server into the process environment, not the prompt
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the HixAI API and GitHub should not have access to arbitrary hosts.
