...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-hixai-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `HIXAI_HOST_PORT` (default `3100`)
- `HIXAI_HOST_FROM_CONTAINER` (default `host.docker.internal`)
...
# or
HIXAI_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```
