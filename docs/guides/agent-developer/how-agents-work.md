...
| `HIXAI_WAKE_REASON` | Why the agent was woken (e.g. `issue_assigned`, `issue_comment_mentioned`) |
| `HIXAI_WAKE_COMMENT_ID` | Specific comment that triggered this wake |
| `HIXAI_APPROVAL_ID` | Approval that was resolved |
| `HIXAI_APPROVAL_STATUS` | Approval decision (`approved`, `rejected`) |
