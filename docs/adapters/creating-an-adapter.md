...
Build a custom adapter to connect HixAI to any agent runtime.
...
2. Build environment with `buildHixAIEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `hixai run --watch` using `picocolors`.
...
Make HixAI skills discoverable to your agent runtime without writing to the agent's working directory:
