...
- Running a Python script that calls the HixAI API
...
1. HixAI spawns the configured command as a child process
2. Standard HixAI environment variables are injected (`HIXAI_AGENT_ID`, `HIXAI_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the HixAI API and perform work.
