...
During heartbeats, agents receive a short-lived JWT via the `HIXAI_API_KEY` environment variable. Use it in the Authorization header:

```
Authorization: Bearer <HIXAI_API_KEY>
```
