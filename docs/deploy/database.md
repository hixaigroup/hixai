...
HixAI uses PostgreSQL via Drizzle ORM. There are three ways to run the database.
...
1. Creates `~/.hixai/instances/default/db/` for storage
2. Ensures the `hixai` database exists
3. Runs migrations automatically
4. Starts serving requests

Data persists across restarts. To reset: `rm -rf ~/.hixai/instances/default/db`.
