...
IMAGE_NAME="${IMAGE_NAME:-hixai-onboard-smoke}"
...
HIXAI_DEPLOYMENT_MODE="${HIXAI_DEPLOYMENT_MODE:-authenticated}"
HIXAI_DEPLOYMENT_EXPOSURE="${HIXAI_DEPLOYMENT_EXPOSURE:-private}"
HIXAI_PUBLIC_URL="${HIXAI_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@hixai.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-hixai-smoke-password}"
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e HIXAI_DEPLOYMENT_MODE="$HIXAI_DEPLOYMENT_MODE" \
      -e HIXAI_DEPLOYMENT_EXPOSURE="$HIXAI_DEPLOYMENT_EXPOSURE" \
      -e HIXAI_PUBLIC_URL="$HIXAI_PUBLIC_URL" \
      -e HIXAI_HOME="/hixai" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "hixai@${PAPERCLIPAI_VERSION}" auth bootstrap-ceo --data-dir "$HIXAI_HOME" --base-url "$HIXAI_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$HIXAI_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$HIXAI_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$HIXAI_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$HIXAI_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$HIXAI_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$HIXAI_PUBLIC_URL/api/companies")"
...
echo "    Public URL: $HIXAI_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $HIXAI_DEPLOYMENT_MODE/$HIXAI_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/hixai-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$HIXAI_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $HIXAI_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$HIXAI_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
