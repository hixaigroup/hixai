...
HIXAI_HOST_PORT="${HIXAI_HOST_PORT:-3100}"
HIXAI_HOST_FROM_CONTAINER="${HIXAI_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.hixai-openclaw.override.yml"
...
detect_hixai_base_url() {
  ...
  for candidate in "$HIXAI_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${HIXAI_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${HIXAI_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
hixai_base_url="$(detect_hixai_base_url || true)"
...
HIxAI URL for OpenClaw container:
EOF
  if [[ -n "$hixai_base_url" ]]; then
    cat <<EOF
  $hixai_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${HIXAI_HOST_PORT}
  (Do not use http://127.0.0.1:${HIXAI_HOST_PORT} inside the container.)
  If HIxAI rejects the host, run on host machine:
    pnpm hixai allowed-hostname host.docker.internal
  Then restart HIxAI and re-run this script.
EOF
  fi
  cat <<EOF
