...
HIXAI_API_URL="${HIXAI_API_URL:-http://127.0.0.1:3100}"
API_BASE="${HIXAI_API_URL%/}/api"
...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-hixai-smoke}"
...
HIXAI_API_URL_FOR_OPENCLAW="${HIXAI_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${HIXAI_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${HIXAI_AUTH_HEADER}" )
fi
if [[ -n "${HIXAI_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${HIXAI_COOKIE}" )
  HIXAI_BROWSER_ORIGIN="${HIXAI_BROWSER_ORIGIN:-${HIXAI_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${HIXAI_BROWSER_ORIGIN}" -H "Referer: ${HIXAI_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set HIXAI_COOKIE or HIXAI_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg hixaiApiUrl "$HIXAI_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/hixai"
  local claimed_file="${workspace_dir}/hixai-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/hixai-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/hixai-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/hixai-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/hixai"
    assert_status "200"
    {
      echo "HIXAI_API_URL=${HIXAI_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
