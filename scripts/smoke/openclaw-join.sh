...
COMPANY_ID="${COMPANY_ID:-${HIXAI_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-hixai-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-hixai-openclaw-smoke}"
...
Provide one of:
  HIXAI_AUTH_HEADER=\"Bearer <board-token>\"
  HIXAI_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "HIxAI OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.hixai.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
