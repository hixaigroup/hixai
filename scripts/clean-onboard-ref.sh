...
Environment overrides:
  ...
  PC_DATA=/tmp/data           HixAI data dir to use
  HIXAI_HOST=127.0.0.1    Host passed to the onboarded server
  HIXAI_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/hixai-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/hixai-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export HIXAI_HOST="${HIXAI_HOST:-127.0.0.1}"
export HIXAI_PORT="${HIXAI_PORT:-3100}"
export HIXAI_OPEN_ON_LISTEN="${HIXAI_OPEN_ON_LISTEN:-false}"
...
echo "HIXAI_HOST: $HIXAI_HOST"
echo "HIXAI_PORT: $HIXAI_PORT"
...
env \
  ...
  HOST="$HIXAI_HOST" \
  PORT="$HIXAI_PORT" \
  HIXAI_OPEN_ON_LISTEN="$HIXAI_OPEN_ON_LISTEN" \
  pnpm hixai onboard --yes --data-dir "$PC_DATA"
