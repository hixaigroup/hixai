...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.HIXAI_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.HIXAI_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.HIXAI_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set HIXAI_COMPANY_ID, or set context profile companyId via `hixai context set`.",
    );
  }
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.HIXAI_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.HIXAI_SERVER_PORT || "");
