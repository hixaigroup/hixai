...
import type { Agent } from "@hixai/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveHixAISkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export HIXAI_API_URL='${escaped(input.apiBase)}'`,
    `export HIXAI_COMPANY_ID='${escaped(input.companyId)}'`,
    `export HIXAI_AGENT_ID='${escaped(input.agentId)}'`,
    `export HIXAI_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local HixAI skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing HixAI skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveHixAISkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local HixAI skills directory. Expected ./skills in the repo checkout.",
              );
            }
