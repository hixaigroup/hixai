...
export const DEFAULT_WORKTREE_HOME = "~/.hixai-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".hixai");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "hixai",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    HIXAI_HOME: paths.homeDir,
    HIXAI_INSTANCE_ID: paths.instanceId,
    HIXAI_CONFIG: paths.configPath,
    HIXAI_CONTEXT: paths.contextPath,
    HIXAI_IN_WORKTREE: "true",
    ...(branding?.name ? { HIXAI_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { HIXAI_WORKTREE_COLOR: branding.color } : {}),
  };
}
