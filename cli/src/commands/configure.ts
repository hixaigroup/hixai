...
export async function configure(opts: {
  ...
  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `hixai onboard` first.");
    p.outro("");
    return;
  }
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because HIXAI_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
