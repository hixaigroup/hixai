...
export async function runCommand(opts: RunOptions): Promise<void> {
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("hixai onboard")} once, then retry ${pc.cyan("hixai run")}.`);
      process.exit(1);
    }
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  if (process.env.HIXAI_UI_DEV_MIDDLEWARE !== undefined) return;
  const normalized = entrypoint.replaceAll("\\", "/");
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@hixai/server/src/index.ts")) {
    process.env.HIXAI_UI_DEV_MIDDLEWARE = "true";
  }
}
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @hixai/server package
  try {
    const mod = await import("@hixai/server");
    return await startServerFromModule(mod, "@hixai/server");
  } catch (err) {
