...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.HIXAI_CONFIG;
...
const WORKTREE_NAME_PREFIX = "hixai-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.HIXAI_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.HIXAI_WORKTREE_START_POINT) ?? undefined;
}
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.hixai"));
...
function resolveSourceConnectionString(config: HIxAIConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://hixai:hixai@127.0.0.1:${port}/hixai`;
}
...
export function copySeededSecretsKey(input: {
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.HIXAI_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.HIXAI_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.HIXAI_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.HIXAI_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  ...
  try {
    ...
    const adminConnectionString = `postgres://hixai:hixai@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "hixai");
    const targetConnectionString = `postgres://hixai:hixai@127.0.0.1:${targetHandle.port}/hixai`;
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const out = {
    ...
    ...(envEntries.HIXAI_INSTANCE_ID ? { HIXAI_INSTANCE_ID: envEntries.HIXAI_INSTANCE_ID } : {}),
    ...(envEntries.HIXAI_CONTEXT ? { HIXAI_CONTEXT: envEntries.HIXAI_CONTEXT } : {}),
    ...envEntries,
  };
...
export function registerWorktreeCommands(program: Command): void {
  ...
  program
    ...
    .option("--home <path>", `Home root for worktree instances (env: HIXAI_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source HIXAI_HOME used when deriving the source config")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with hixai- if needed")
