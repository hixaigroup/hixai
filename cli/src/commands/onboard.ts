...
const ONBOARD_ENV_KEYS = [
  "HIXAI_PUBLIC_URL",
  "DATABASE_URL",
  "HIXAI_DB_BACKUP_ENABLED",
  "HIXAI_DB_BACKUP_INTERVAL_MINUTES",
  "HIXAI_DB_BACKUP_RETENTION_DAYS",
  "HIXAI_DB_BACKUP_DIR",
  "HIXAI_DEPLOYMENT_MODE",
  "HIXAI_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "HIXAI_ALLOWED_HOSTNAMES",
  "HIXAI_AUTH_BASE_URL_MODE",
  "HIXAI_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "HIXAI_STORAGE_PROVIDER",
  "HIXAI_STORAGE_LOCAL_DIR",
  "HIXAI_STORAGE_S3_BUCKET",
  "HIXAI_STORAGE_S3_REGION",
  "HIXAI_STORAGE_S3_ENDPOINT",
  "HIXAI_STORAGE_S3_PREFIX",
  "HIXAI_STORAGE_S3_FORCE_PATH_STYLE",
  "HIXAI_SECRETS_PROVIDER",
  "HIXAI_SECRETS_STRICT_MODE",
  "HIXAI_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const publicUrl =
    process.env.HIXAI_PUBLIC_URL?.trim() ||
    process.env.HIXAI_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.HIXAI_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.HIXAI_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.HIXAI_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.HIXAI_ALLOWED_HOSTNAMES
    ? process.env.HIXAI_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.HIXAI_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.HIXAI_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.HIXAI_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.HIXAI_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.HIXAI_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.HIXAI_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.HIXAI_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.HIXAI_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.HIXAI_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.HIXAI_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.HIXAI_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.HIXAI_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.HIXAI_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.HIXAI_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.HIXAI_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "HIXAI_DEPLOYMENT_EXPOSURE",
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@hixai/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `hixai doctor`"));
      }
    }
  ...
  if (jwtSecret.created) {
    p.log.success(`Created ${pc.cyan("HIXAI_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  } else if (process.env.HIXAI_AGENT_JWT_SECRET?.trim()) {
    p.log.info(`Using existing ${pc.cyan("HIXAI_AGENT_JWT_SECRET")} from environment`);
  } else {
  ...
  p.note(
    [
      ...
      "Agent auth: HIXAI_AGENT_JWT_SECRET configured",
    ].join("\n"),
    "Configuration saved",
  );
  ...
  p.note(
    [
      `Run: ${pc.cyan("hixai run")}`,
      `Reconfigure later: ${pc.cyan("hixai configure")}`,
      `Diagnose setup: ${pc.cyan("hixai doctor")}`,
    ].join("\n"),
    "Next commands",
  );
  ...
  if (shouldRunNow && !opts.invokedByRun) {
    process.env.HIXAI_OPEN_ON_LISTEN = "true";
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("hixai run")}`,
        `Then: ${pc.cyan("hixai auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
