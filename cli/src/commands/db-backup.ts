...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@hixai/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://hixai:hixai@127.0.0.1:${port}/hixai`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "hixai";
