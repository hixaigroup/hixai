...
function createTempConfig(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "hixai-doctor-"));
  const configPath = path.join(root, ".hixai", "config.json");
...
describe("doctor", () => {
  ...
  it("re-runs repairable checks so repaired failures do not remain blocking", async () => {
    ...
    expect(process.env.HIXAI_AGENT_JWT_SECRET).toBeTruthy();
  });
});
