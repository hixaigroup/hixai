...
export function applyDataDirOverride(
  ...
  process.env.HIXAI_HOME = resolvedDataDir;
  ...
  if (support.hasContextOption) {
    const hasContextOverride = Boolean(options.context?.trim()) || Boolean(process.env.HIXAI_CONTEXT?.trim());
    if (!hasContextOverride) {
      process.env.HIXAI_CONTEXT = resolveDefaultContextPath();
    }
  }
