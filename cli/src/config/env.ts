...
const JWT_SECRET_ENV_KEY = "HIXAI_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# HixAI environment variables",
    "# Generated by HixAI CLI commands",
...
export function resolveHixAIEnvFile(configPath?: string): string {
...
export function loadHixAIEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeHixAIEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readHixAIEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeHixAIEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeHixAIEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readHixAIEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeHixAIEnvEntries(next, filePath);
  return next;
}
